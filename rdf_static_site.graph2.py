...
def run_classindex_mode(args) -> int:
    ...
    legacy TopBraid servlet workflow. It links to per-instance pages named
    <LocalName>.html (matching instancepages naming).
    """
    ...
    for s in instances:
        ...
        # Build href to match where per-instance HTML is deployed.
        # In the publish workflow, per-instance HTML is uploaded under:
        #   /vocab/<prefix>/<LocalName>.html
        vocab_dir = None
        m = re.search(r"/vocab/([^/]+)/", iri_s)
        if m:
            vocab_dir = m.group(1)

        if vocab_dir:
            # Prefer the CURIE local part when the prefix matches the vocab dir
            if parts and parts[0] == vocab_dir:
                href = f"/vocab/{vocab_dir}/{parts[2]}{args.href_suffix}"
            else:
                href = f"/vocab/{vocab_dir}/{local_name}{args.href_suffix}"
        else:
            href = f"{local_name}{args.href_suffix}"
