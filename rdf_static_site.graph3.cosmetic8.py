...
QUDT_LOGO_URL = "https://qudt.org/linkedmodels.org/assets/lib/lm/images/logos/qudt_logo-300x110.png"
...
<body>
<header class="top">
  <a class="brand" href="https://qudt.org/"><img class="logo" src="{{ logo_src }}" alt="QUDT logo"></a>
  <div class="head">
...
def _render_values_list_html(values: List[str]) -> str:
    """Render multiple object values for one predicate as a compact list.

    Uses native <details> to avoid heavy JS dependencies.
    `values` are already HTML strings and should be treated as safe.
    """
    n = len(values)
    lis = "\n".join(f"<li>{v}</li>" for v in values)
    ul = f"<ul class='values'>\n{lis}\n</ul>"

    # Use a collapsible control for longer lists (e.g., applicableUnit).
    if n >= 8:
        return f"<details class='values-details'><summary>Show {n} values</summary>{ul}</details>"
    return ul


def _group_predicate_runs(triples: List[Tuple[str, str]]) -> List[Dict[str, str]]:
    """Group consecutive identical predicates (for non-bnode rows) so the predicate isn't repeated.

    We intentionally only group *consecutive* runs to preserve the bnode expansion structure.
    """
    ...
    while i < len(triples):
        ...
        j = i + 1
        vals = [triples[i][1]]
        while j < len(triples) and triples[j][0] == p:
            vals.append(triples[j][1])
            j += 1

        # Don't group:
        # - bnode placeholder rows (empty object),
        # - expansion rows ("↳ ..."),
        # - runs that include any empty object.
        if p.lstrip().startswith("↳") or any(v == "" for v in vals) or len(vals) == 1:
            for k in range(i, j):
                rows.append({"p": triples[k][0], "o": triples[k][1]})
        else:
            rows.append({"p": p, "o": _render_values_list_html(vals)})
...
def write_instance_page(
    *,
    ...
    raw_triples = subject_rows(g, s, prefix_map, gospel, bnode_expand_depth=bnode_expand_depth)
    rows = _group_predicate_runs(raw_triples)
...
def run_classindex_mode(args) -> int:
    """Generate an index-style HTML page for all instances of a class.

    This is the file-based replacement for the lmdoc:Graph output used in the
    legacy TopBraid servlet workflow. It links to canonical QUDT resource IRIs (e.g., http://qudt.org/vocab/unit/A).
    """
    ...
    for s in instances:
        parts = try_curie_parts(g, s, prefix_map)
        iri_s = str(s)
        local_name = iri_local_name(iri_s)
        # Link to the canonical resource IRI (same behavior as replacement links for deprecated units).
        # Example: unit:A -> http://qudt.org/vocab/unit/A
        href = iri_s
    ...
    if not getattr(args, "quiet", False):
        print(f"Wrote class index ({len(items)} items): {out_path.resolve()}")
    return 0
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    New style:
      ...
      rdf_static_site.py classindex --input INPUT.ttl --class CLASS_IRI --output OUT.html
    """
    import sys
    ...
    # classindex
    ap_ci = sub.add_parser("classindex", help="Generate an index-style HTML page for instances of a class (no per-instance pages).")
    ap_ci.add_argument("--input", required=True, help="Input RDF file (typically Turtle) containing the instances")
    ap_ci.add_argument("--class", dest="cls", required=True, help="Class IRI whose instances should be listed")
    ap_ci.add_argument("--output", required=True, help="Output HTML file path")
    ap_ci.add_argument("--title", default="RDF Index", help="Title for the index page")
    ap_ci.add_argument("--href-suffix", default=".html", help="Suffix for instance links (default: .html)")
    ap_ci.add_argument("--format", default=None, help="Optional RDFLib parser format (default: autodetect)")
    ap_ci.add_argument("--quiet", action="store_true", help="Suppress progress output")
    ...
    if args.cmd == "classindex":
        return run_classindex_mode(args)
